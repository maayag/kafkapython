...
class KafkaConsumer:
    ...
    def __init__(
        ...
        if is_avro is True: 
            self.broker_properties["schema.registry.url"] = "http://localhost:48081"
    ...
    def _consume(self):
        """Pools for a message. Returns 1 if a message was received, 0 otherwise"""
        while True:
            message = self.consume.poll(self.consume.consume_timeout)
            if message is None:
